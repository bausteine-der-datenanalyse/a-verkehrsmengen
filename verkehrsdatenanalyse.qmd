...
ggplot(data = ZST5116_DTV, aes(x = Stunde, y = avg_KFZ_R1 ,group = Wotag ,color = factor(Wotag), shape = Wotag)) +
  geom_point(size = 2) +
...
```{r}
ggplot(data = data_dienstag_mean, aes(x = Stunde, y = Anteil, group = Zählstelle, color = Zählstelle, shape = Zählstelle)) +
  geom_point(size = 2) +
  geom_line(linewidth = 1) +
...
read_unfaelle <- function(f) {
  read_csv2(f, id = "Jahr", locale = locale(encoding = 'iso-8859-1')) |>
 filter(DTV_Kfz_MobisSo_Q != 'NA' & DTV_SV_MobisSo_Q != 'NA') 
}

data_na <- list.files(path="data_jw", pattern = "\\.csv$",full.names=T) |>
  map(read_unfaelle) |>
  list_rbind() |>
  str_remove_all(Jahr, "[data_jw/Jawe]") |>
  str_remove_all(Jahr, "[.csv]") |>
  as.numeric(Jahr)
